package com.hunter.adapter.callback;
...
import com.hunter.adapter.BaseItemDraggableAdapter;
import com.hunter.adapter.R;
...
public class ItemDragAndSwipeCallback extends ItemTouchHelper.Callback {
    ...
    public void clearView(RecyclerView recyclerView, RecyclerView.ViewHolder viewHolder) {
        ...
        if (viewHolder.itemView.getTag(
                R.id.BaseQuickAdapter_dragging_support) != null && (Boolean) viewHolder.itemView.getTag(
                R.id.BaseQuickAdapter_dragging_support)) {
            mAdapter.onItemDragEnd(viewHolder);
            viewHolder.itemView.setTag(R.id.BaseQuickAdapter_dragging_support, false);
        }
        if (viewHolder.itemView.getTag(
                R.id.BaseQuickAdapter_swiping_support) != null && (Boolean) viewHolder.itemView.getTag(
                R.id.BaseQuickAdapter_swiping_support)) {
    ...
    @Override
    public void onMoved(RecyclerView recyclerView, RecyclerView.ViewHolder source, int fromPos,
     ...
     * <p>The value should be ItemTouchHelper.UP, ItemTouchHelper.DOWN, ItemTouchHelper.LEFT, ItemTouchHelper.RIGHT
     * or their combination.</p>
     * You can combine them like ItemTouchHelper.UP | ItemTouchHelper.DOWN, it means that the item could only move up
     * and down when dragged.
     *
     * @param dragMoveFlags the drag movement direction. Default value is ItemTouchHelper.UP | ItemTouchHelper.DOWN |
     ...
     * You can combine them like ItemTouchHelper.START | ItemTouchHelper.END, it means that the item could swipe to
     * both left or right.
     *
    ...
    private boolean isViewCreateByAdapter(RecyclerView.ViewHolder viewHolder) {
        ...
        if (type == mAdapter.HEADER_VIEW || type == mAdapter.LOADING_VIEW || type == mAdapter.FOOTER_VIEW || type ==
                mAdapter.EMPTY_VIEW) {
            return true;
        }
        return false;
