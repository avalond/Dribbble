...
import static com.hunter.adapter.BaseQuickAdapter.EMPTY_VIEW;
import static com.hunter.adapter.BaseQuickAdapter.FOOTER_VIEW;
import static com.hunter.adapter.BaseQuickAdapter.HEADER_VIEW;
import static com.hunter.adapter.BaseQuickAdapter.LOADING_VIEW;
    ...
    public boolean onInterceptTouchEvent(RecyclerView rv, MotionEvent e) {
        ...
        if (recyclerView == null) {
            ...
            mGestureDetector = new GestureDetectorCompat(recyclerView.getContext(),
                                                         new ItemTouchHelperGestureListener(recyclerView));
        }
                    ...
                    for (Iterator it = childClickViewIds.iterator(); it.hasNext(); ) {
                        ...
                        if (inRangeOfView(childView, e) && childView.isEnabled()) {
                            onItemChildClick(baseQuickAdapter, childView,
                                             vh.getLayoutPosition() - baseQuickAdapter.getHeaderViewCount());
                            resetPressedView(pressedView);
                            return true;
                        }
                    }

                    onItemClick(baseQuickAdapter, pressedView,
                                vh.getLayoutPosition() - baseQuickAdapter.getHeaderViewCount());
                } else {
                    onItemClick(baseQuickAdapter, pressedView,
                                vh.getLayoutPosition() - baseQuickAdapter.getHeaderViewCount());
                ...
                if (!isHeaderOrFooterPosition(vh.getLayoutPosition())) {
                    ...
                    if (longClickViewIds != null && longClickViewIds.size() > 0) {
                        for (Iterator it = longClickViewIds.iterator(); it.hasNext(); ) {
                            ...
                            if (inRangeOfView(childView, e) && childView.isEnabled()) {
                                onItemChildLongClick(baseQuickAdapter, childView,
                                                     vh.getLayoutPosition() - baseQuickAdapter.getHeaderViewCount());
                                mPressedView.setPressed(true);
                                mIsShowPress = true;
                    ...
                    if (!isChildLongClick) {
                        onItemLongClick(baseQuickAdapter, mPressedView,
                                        vh.getLayoutPosition() - baseQuickAdapter.getHeaderViewCount());
                        mPressedView.setPressed(true);
                        mIsShowPress = true;
                    }
